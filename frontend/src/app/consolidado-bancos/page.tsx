...
export default function ConsolidadoBancosPage() {
  ...
  async function descargarPDF() {
    ...
    try {
      ...
      const canvas = await (window as any).html2canvas(printRef.current, { scale: 2, useCORS: true, backgroundColor: '#333F50', logging: false });
    ...
    <div className="min-h-screen bg-[#333F50]">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-4">
        <div className="bg-[#3C4759] rounded-2xl border border-[#4A5568] shadow-sm p-4">
          <p className="text-[11px] font-black text-[#C9D2DE] uppercase tracking-wider mb-2">Ir al estado de cuenta</p>
          <div className="relative">
            <div className="flex items-center gap-2 border-2 border-[#16a34a] bg-[#2B3547] rounded-xl px-3 py-2.5">
              ...
              <input
                ...
                className="flex-1 bg-transparent text-sm font-semibold text-white placeholder:text-white/40 focus:outline-none"
              />
              {busqCod && (
                <button onClick={() => { setBusqCod(''); setBusqResultados([]); }}
                  className="text-[#C9D2DE] hover:text-white text-lg leading-none">×</button>
              )}
            </div>

            {/* Resultados */}
            {busqResultados.length > 0 && (
              <div className="absolute z-20 left-0 right-0 mt-1 bg-[#3C4759] rounded-xl border border-[#4A5568] shadow-lg overflow-hidden">
                {busqResultados.map(d => (
                  <button key={d.id}
                    onClick={() => { router.push(`/alumnos/${d.id}/estado-cuenta`); setBusqCod(''); setBusqResultados([]); }}
                    className="w-full flex items-center gap-3 px-4 py-3 hover:bg-[#2B3547] transition text-left border-b border-[#4A5568] last:border-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-black text-white text-sm truncate">{d.nombre}</p>
                      <p className="text-[11px] text-[#C9D2DE] font-semibold">Código: {d.codigo || '—'}</p>
                    </div>
            ...
            {busqCod && busqResultados.length === 0 && (
              <div className="absolute z-20 left-0 right-0 mt-1 bg-[#3C4759] rounded-xl border border-[#4A5568] shadow-lg px-4 py-3">
                <p className="text-sm text-[#C9D2DE] font-semibold">Sin resultados para "{busqCod}"</p>
              </div>
            )}
          </div>
        </div>
      ...
      {errorLS && (
        <div className="max-w-2xl mx-auto mt-4 px-4">
          <div className="bg-[#2B3547] border border-[#C0504D] rounded-xl p-4 text-sm text-white font-semibold">{errorLS}</div>
        </div>
      )}

      {lotes.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-32 text-center px-4">
          <Landmark className="w-16 h-16 text-[#4A5568] mb-4" />
          <p className="text-[#C9D2DE] font-black text-lg">Sin registros aún</p>
          <p className="text-[#C9D2DE] text-sm mt-1 mb-6">Sube un extracto bancario y aplica los pagos para verlos aquí.</p>
        ...
        <main className="max-w-6xl mx-auto px-4 sm:px-6 py-5 space-y-5" ref={printRef}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { label: 'Errores',         val: String(totalError),     sub: 'código no encontrado',     color: '#C0504D' },
              { label: 'Lotes cargados',  val: String(lotes.length),   sub: `${todasLineas.length} registros totales`, color: '#FFFFFF' },
            ].map(c => (
              <div key={c.label} className="bg-[#3C4759] rounded-xl border border-[#4A5568] shadow-sm p-4 text-center">
                <p className="text-[11px] font-black text-[#C9D2DE] uppercase mb-1">{c.label}</p>
                <p className="text-lg font-black" style={{ color: c.color }}>{c.val}</p>
                <p className="text-[10px] text-[#C9D2DE]">{c.sub}</p>
              </div>
            ))}
          </div>
          ...
          {porConcepto.length > 0 && (
            <div className="bg-[#3C4759] rounded-2xl border border-[#4A5568] shadow-sm overflow-hidden">
              <div className="px-5 py-3 border-b border-[#4A5568]">
                <p className="font-black text-white text-sm">Resumen por concepto</p>
                <p className="text-[11px] text-[#C9D2DE]">Base contable — totales recaudados</p>
              </div>
                  ...
                  <tbody>
                    {porConcepto.map(row => (
                      <tr key={row.concepto} style={{ background: ROW }}>
                        <td style={{ padding: '6px 12px', borderRight: BW, borderBottom: BW, fontWeight: 900, color: '#5BE39B' }}>{row.concepto}</td>
                        <td style={{ padding: '6px 12px', borderRight: BW, borderBottom: BW, textAlign: 'right', color: '#FFFFFF', fontWeight: 700 }}>{row.count}</td>
                        <td style={{ padding: '6px 12px', borderBottom: BW, textAlign: 'right', color: '#5BE39B', fontWeight: 900 }}>{formatPeso(row.pagado)}</td>
                      </tr>
                    ))}
                    <tr>
                      <td style={{ padding: '7px 12px', borderRight: BW, fontWeight: 900, background: '#2A3342', color: '#FFFFFF' }}>TOTAL</td>
                      <td style={{ padding: '7px 12px', borderRight: BW, textAlign: 'right', background: '#2A3342', fontWeight: 900, color: '#FFFFFF' }}>{porConcepto.reduce((s, r) => s + r.count, 0)}</td>
                      <td style={{ padding: '7px 12px', textAlign: 'right', background: '#2A3342', fontWeight: 900, color: '#5BE39B' }}>{formatPeso(porConcepto.reduce((s, r) => s + r.pagado, 0))}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
          ...
          <div className="bg-[#3C4759] rounded-2xl border border-[#4A5568] shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-[#4A5568] flex flex-wrap gap-2 items-center">
              <p className="font-black text-white text-sm flex-1">Detalle de pagos</p>
              <div className="flex items-center gap-1.5 border border-[#4A5568] bg-[#2B3547] rounded-lg px-2 py-1.5">
                <Search className="w-3 h-3 text-[#C9D2DE]" />
                <input value={filtroCod} onChange={e => setFiltroCod(e.target.value)}
                  placeholder="Código..."
                  className="text-xs w-20 bg-transparent focus:outline-none text-white placeholder:text-white/40" />
              </div>
              <select value={filtroConc} onChange={e => setFiltroConc(e.target.value)}
                className="border border-[#4A5568] rounded-lg px-2 py-1.5 text-xs font-semibold text-white focus:outline-none bg-[#2B3547]">
              ...
              <span className="text-[11px] text-[#C9D2DE]">{lineaFiltrada.length} registros</span>
            </div>
              ...
              <table className="w-full text-xs border-collapse" style={{ minWidth: 1050 }}>
                ...
                <tbody>
                  {lineaFiltrada.length === 0 ? (
                    <tr><td colSpan={12} style={{ padding: '24px', textAlign: 'center', color: '#C9D2DE', fontWeight: 600 }}>Sin registros con estos filtros</td></tr>
                    ...
                    return (
                      <tr key={i} style={{ background: ROW }}>
                        ...
                        <td style={{ padding: '5px 10px', borderRight: BW, borderBottom: BW, color: '#C9D2DE', whiteSpace: 'nowrap' }}>{formatFecha(ln.fecha)}</td>
                        <td style={{ padding: '5px 10px', borderRight: BW, borderBottom: BW, color: '#FFFFFF' }}>{ln.descripcion||'—'}</td>
                        <td style={{ padding: '5px 10px', borderRight: BW, borderBottom: BW, textAlign: 'right', fontWeight: 900, color: '#FFFFFF', whiteSpace: 'nowrap' }}>{ln.debito ? formatPeso(ln.debito) : '—'}</td>
                        ...
                        <td style={{ padding: '5px 10px', borderRight: BW, borderBottom: BW, color: '#C9D2DE', whiteSpace: 'nowrap' }}>{ln.saldo||'—'}</td>
                        <td style={{ padding: '5px 10px', borderRight: BW, borderBottom: BW, color: '#C9D2DE', whiteSpace: 'nowrap' }}>{ln.cuenta||'—'}</td>
          ...
          <div className="bg-[#3C4759] rounded-2xl border border-[#4A5568] shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-[#4A5568]">
              <p className="font-black text-white text-sm">Historial de lotes cargados</p>
              <p className="text-[11px] text-[#C9D2DE]">{lotes.length} lote{lotes.length !== 1 ? 's' : ''}</p>
            </div>
            <div className="divide-y divide-[#4A5568]">
              {lotes.map(lote => {
                ...
                return (
                  <div key={lote.id}>
                    <div className="flex items-center gap-3 px-5 py-3 hover:bg-[#2B3547] transition">
                      <button onClick={() => setLoteAbierto(abierto ? null : lote.id)}
                        className="flex items-center gap-2 flex-1 text-left">
                        {abierto ? <ChevronDown className="w-4 h-4 text-[#C9D2DE] flex-shrink-0"/> : <ChevronRight className="w-4 h-4 text-[#C9D2DE] flex-shrink-0"/>}
                        <div>
                          <p className="font-black text-white text-sm">{formatFechaISO(lote.fechaSubida)}</p>
                          <p className="text-[11px] text-[#C9D2DE]">{lote.totalRegistros} registros · {lineasNorm.filter(l => l.estado === 'PAGÓ' || l.estado === 'PAGÓ CON 10%').length} pagados · {lineasNorm.filter(l => l.estado === 'VERIFICAR').length} verificar · {lineasNorm.filter(l => l.estado === 'ERROR').length} errores</p>
                        </div>
                      </button>
                      <button onClick={() => eliminarLote(lote.id)}
                        className="p-1.5 rounded-lg text-[#C9D2DE] hover:text-[#C0504D] hover:bg-[#2B3547] transition flex-shrink-0">
                        <Trash2 className="w-4 h-4"/>
                      </button>
                    </div>
                    {abierto && (
                      <div className="overflow-auto border-t border-[#4A5568] bg-[#2B3547]" style={{ maxHeight: 280 }}>
                          ...
                          <thead>
                            <tr>
                              {['ESTADO','CÓDIGO','DEPORTISTA','DETALLE','DÉBITO','V. CARGADO','NOTA'].map(h => (
                                <th key={h} style={{ background: '#2A3342', color: 'white', padding: '5px 10px', textAlign: ['DÉBITO','V. CARGADO'].includes(h) ? 'right' : 'left', fontWeight: 900, borderRight: BW, whiteSpace: 'nowrap', fontSize: 10 }}>{h}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
                            {lineasNorm.map((ln, i) => {
                              ...
                              return (
                                <tr key={i} style={{ background: ROW }}>
                                  ...
                                  <td style={{ padding: '4px 10px', borderRight: BW, borderBottom: BW, fontWeight: 900, color: '#5BE39B' }}>{ln.codigo||'—'}</td>
                                  <td style={{ padding: '4px 10px', borderRight: BW, borderBottom: BW, color: '#FFFFFF' }}>{ln.deportista||'—'}</td>
                                  <td style={{ padding: '4px 10px', borderRight: BW, borderBottom: BW, color: '#FFFFFF' }}>{ln.detalle||'—'}</td>
                                  <td style={{ padding: '4px 10px', borderRight: BW, borderBottom: BW, textAlign: 'right', fontWeight: 900, color: '#FFFFFF' }}>{ln.debito ? formatPeso(ln.debito) : '—'}</td>
                                  <td style={{ padding: '4px 10px', borderRight: BW, borderBottom: BW, textAlign: 'right', color: '#C9D2DE' }}>{ln.vCargado ? formatPeso(ln.vCargado) : '—'}</td>
                                  <td style={{ padding: '4px 10px', borderBottom: BW, color: '#C9D2DE', fontSize: 10 }}>{ln.mensaje||'—'}</td>
                                </tr>
                              );
                            })}
                          </tbody>
